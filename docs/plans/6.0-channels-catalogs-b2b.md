...
- **`Spree::Channel` itself ships in 5.5** as part of `6.0-order-routing.md`. The model in 5.5 is the minimal shape (store, name, code, active, preferences, metafields) — exactly what's specified in the Channel section below, minus `default_catalog_id` (no `Catalog` yet). 6.0 layers on `default_catalog_id`, `default_stock_location_id`, and the `has_many :product_listings` association without needing migrations against the table — only `add_reference` migrations to add new FKs.
- **`Order.channel_id` FK ships in 5.5** (the deprecated string column is dropped in the same migration). 6.0 doesn't need to migrate Orders — just adds `company_location_id`.
