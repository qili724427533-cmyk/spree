...
**Status:** Draft (Channel itself ships in 5.5 with Order Routing; ProductListing, Catalog, Company tree, pricing integration land in 6.0)
**Target:** Spree 6.0 (5.5 ships only the `Spree::Channel` model — see `6.0-order-routing.md`)
...
- Drop `spree_orders.channel` string column (5.5+ already ignores it via `ignored_columns`; 5.5 ships a backfill rake that copies values into `channel_id`)

## Constraints on Current Work

- **`Spree::Channel` itself ships in 5.5** as part of `6.0-order-routing.md`. The model in 5.5 is the minimal shape (store, name, code, active, preferences, metafields) — exactly what's specified in the Channel section below, minus `default_catalog_id` (no `Catalog` yet) and minus `default_stock_location_id` (defined in `6.0-order-routing.md`'s Phase 2). 6.0 layers these on with `add_reference` migrations against `spree_channels` (`default_catalog_id`, `default_stock_location_id`) plus the `has_many :product_listings` association and the new `spree_product_listings` table. No table-level alterations to existing columns.
- **`Order.channel_id` FK ships in 5.5** alongside the legacy string column. The 5.5 upgrade rake `spree:channels:upgrade` (which composes `create_defaults` + `backfill_order_channel_ids`) populates the FK; `Spree::Order` ignores the string column via `ignored_columns`. The string column is dropped in 6.1 cleanup. 6.0 doesn't need to migrate Orders — just adds `company_location_id`.
